...
///    [`forge_app::fire_worktree_create_hook`] so plugins can veto
///    the creation or hand back a custom path.
/// 2. Prints a user-facing status title with
///    [`TitleFormat::info`] — something the manager itself must
///    not do because the manager is shared with the future
///    runtime `EnterWorktreeTool` path (deferred).
...
impl<'a, S: Services + 'static> Sandbox<'a, S> {
    ...
    ///
    /// - If a `WorktreeCreate` plugin hook sets `blocking_error`, the
    ///   creation is aborted with that error message.
    /// - If a plugin hook provides a `worktreePath` override, that
    ///   path is validated (must exist) and canonicalized; the
    ///   built-in `git worktree add` path is skipped entirely.
    /// - Otherwise the manager's `git worktree add` path runs
    ///   normally and the resulting path is returned.
    ///
