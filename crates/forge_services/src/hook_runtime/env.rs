...
/// - `FORGE_PLUGIN_ROOT` — path to the current plugin's directory (only
///   set when the hook originates from a plugin).
/// - `FORGE_PLUGIN_DATA` — `<forge_home>/plugin-data/<plugin-name>/`
///   (only set when `plugin_name` is provided). The caller is
///   responsible for creating this directory.
/// - `FORGE_PLUGIN_OPTION_<KEY>` — one per user-configured plugin
///   option. Keys are upper-cased and hyphens are replaced with
///   underscores.
/// - `FORGE_SESSION_ID` — current session ID.
/// - `FORGE_ENV_FILE` — temp file path that `SessionStart`/`Setup`
///   hooks write `export FOO=bar` lines into.
///
/// `plugin_options` is a slice of `(key, value)` pairs rather than a
/// `HashMap` so the caller controls iteration order (useful for
/// deterministic test assertions).
pub fn build_hook_env_vars(
    project_dir: &Path,
...
mod tests {
    ...
    fn test_minimal_inputs_produce_three_core_vars() {
        ...
        assert_eq!(actual.get("FORGE_PROJECT_DIR").map(String::as_str), Some("/proj"));
        assert_eq!(actual.get("FORGE_SESSION_ID").map(String::as_str), Some("sess-1"));
        assert_eq!(actual.get("FORGE_ENV_FILE").map(String::as_str), Some("/tmp/env"));
    ...
    fn test_plugin_options_are_upper_cased_and_hyphens_normalized() {
        ...
        assert_eq!(
            actual.get("FORGE_PLUGIN_OPTION_API_KEY").map(String::as_str),
            Some("secret")
        );
        assert_eq!(
            actual.get("FORGE_PLUGIN_OPTION_LOG_LEVEL").map(String::as_str),
            Some("debug")
        );
    }
}
